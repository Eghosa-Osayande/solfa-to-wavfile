...
class AudioSegment(object):
    ...
    @property
    def sndarray(self):
        sndarr=y=self.get_array_of_samples()
        sndlist=sndarr.tolist()
        sndnd=np.array(sndlist)
        return sndnd.reshape(int(len(sndlist)/self.channels),self.channels)
     
    @staticmethod          
    def load_ogg(filename):
        import pygame as pg
        pg.mixer.init()
        arr=pg.sndarray.array(pg.mixer.Sound(filename))
        snd=AudioSegment.silent()
        snd=snd.set_channels(arr.shape[1])
        snd=snd.new_data(arr)
        return snd
    
    def transpose(self,step=0):
        
        def speedx(snd_array, n):
            """ Speeds up / slows down a sound, by some factor. """
            factor = float(2**(1.0 * n / 12.0))
        ...
        size=arr.shape[0]
        arr=speedx(arr,step)
        return self.new_data(arr)
        
        #return self.set_frame_rate(int(self.frame_rate*2**(-(step)/12)))
        
    def new_data(self,arr):
        sndlist=arr.reshape(1,int(arr.shape[0]*arr.shape[1])).tolist()[0]
        try:
            initial_arr=array.array(self.array_type,sndlist)
        except:
            initial_arr=array.array('i',sndlist)
        return self._spawn(initial_arr)
        '''self.__class__(
            data=initial_arr,
